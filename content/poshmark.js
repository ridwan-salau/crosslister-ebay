...
(async function init() {
  ...
  safeSendMessage({ action: 'GET_STAGED_LISTING' }, async (response) => {
    ...
    bannerObj.el.onclick = () => doFill(item, bannerObj);

    // Auto-fill if form is already visible
    setTimeout(() => {
      const titleEl = document.querySelector(poshmarkConfig.selectors.text.title);
      const descEl = document.querySelector(poshmarkConfig.selectors.text.description);
      if (titleEl || descEl) doFill(item, bannerObj);
    }, 3000);
  });
  ...
  async function doFill(item, bannerObj) {
    ...
    const result = await fillForm(poshmarkConfig, item, settings);
    ...
    showToast(`✓ ${result.filledCount} fields pasted — review and publish on Poshmark.`);
